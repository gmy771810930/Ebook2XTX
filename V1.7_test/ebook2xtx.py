...
支持输出电子书格式：EPUB, PDF
新增：锐化、对比度、CLAHE、抖动算法选择、位深度可调(1-16)、抖动强度百分比
"""
...
def get_user_settings():
    ...
    # 新增：锐化
    sharpen = get_percent_input("请输入锐化强度 (0-100%):", 0)
    # 新增：对比度
    contrast = get_percent_input("请输入对比度强度 (0-100%):", 0)
    # 新增：局部对比度增强 (CLAHE)
    clahe = get_percent_input("请输入局部对比度增强强度 (0-100%):", 0)

    # 抖动算法
    print("\n请选择抖动算法：")
    dither_options = {1: "Floyd-Steinberg (默认)", 2: "Atkinson", 3: "无抖动"}
    dither_choice = get_user_choice("", dither_options, 1)
