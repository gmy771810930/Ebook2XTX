...
"""
漫画/电子书转 XTC/XTCH/XTG/XTH 格式 (Xteink 设备) - Ebook2XTX
优化版：多进程并行处理图片 + Numba 加速抖动 + 自动分包
支持单页输出模式、画面切割、滚动切割、GIF动图、多种图片格式
支持 EPUB/MOBI/AZW3/PDF 等电子书格式（后续扩展）
修改 支持进度回调，便于图形界面集成
...
def get_user_settings():
    ...
    rotate_options = {1: "顺时针90度", 2: "逆时针90度", 3: "不旋转"}
...
def process_archive(archive_path: Path, settings: dict, output_base_dir: Path,
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if has_gif and _gif_mode is None:
            print("\n检测到压缩包中包含GIF动图文件。")
            gif_options = {1: "只处理第一帧（默认）", 2: "处理所有帧（将每一帧作为独立图片输出）"}
...
def main():
    print("="*50)
    print("Ebook2XTX - 电子书转 XTC/XTCH/XTG/XTH 格式 (Xteink 设备)")
    print("="*50)
