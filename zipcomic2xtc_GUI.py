...
"""
漫画压缩包转换工具 - 图形化界面 (独立于控制台版本)
文件名: zipcomic2xtc_GUI.py
基于 tkinter，与 zipcomic2xtc.py 共享核心处理逻辑
布局：
- 目录设置（固定顶部）
...
# 导入转换模块
from zipcomic2xtc import convert_archives, parse_size_string
import zipcomic2xtc  # 用于依赖检查等

# ========== 日志重定向到 GUI ==========
class QueueHandler(logging.Handler):
    """将日志记录发送到队列，供 GUI 线程处理"""
...
class ConverterGUI:
    VERSION = "1.4"
    GITHUB_URL = "https://github.com/gmy771810930/ZipComic2XTC"

    def __init__(self):
        self.root = tk.Tk()
        self.root.title("ZipComic2XTC")
        self.root.geometry("1100x750")
        self.root.minsize(900, 700)  # 最小尺寸保证顶部和底部控件可见
    ...
    def build_ui(self):
        """构建界面：固定顶部 + 中间 PanedWindow（左选项右日志） + 固定底部"""
        # 主框架
        ...
        paned.add(left_scroll, weight=2)  # 权重更大，优先分配空间

        # ---- 右侧：日志区域（直接使用 ScrolledText，自带滚动条） ----
        ...
        paned.add(log_frame, weight=1)  # 权重小，优先被压缩
    ...
    def start_conversion(self):
        ...
        if not output_dir:
            ...
            return
        if not zipcomic2xtc.check_and_install_dependencies():
    ...
    def show_about(self):
        ...
        ttk.Label(frame, text="ZipComic2XTC", font=("微软雅黑", 16, "bold")).pack(pady=5)
        ttk.Label(frame, text=f"版本 {self.VERSION}").pack(pady=2)
        ttk.Label(frame, text="漫画压缩包转 Xteink 设备格式工具").pack(pady=2)
